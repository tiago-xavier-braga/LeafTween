...
A 10-step ramp moving from cool, dark tones into warm, hot ones. Useful
anywhere a sequential scale is needed (severity, intensity, progress) as well
as for general UI theming.
...
| Surface / panel    | `dark-teal`       |
| Primary text (on dark) | `vanilla-custard` |
| Secondary text (on dark) | `pearl-aqua` |
| Primary accent / CTA | `golden-orange` |
| Info                | `dark-cyan`       |
| Warning             | `burnt-caramel`   |
| Error               | `rusty-spice`     |
| Critical / danger   | `brown-red`       |
...
- Verify any final text/background pairing against WCAG AA (4.5:1 for body
  text, 3:1 for large text) before shipping.
